...
public class KeyboardManager extends EventDispatcher implements KeyListener{
	...
	public void keyPressed(KeyEvent e) {
		if(!_keys.contains(e.getKeyCode())){
			...
			this.dispatchEvent(new EventKeyboard(FrameEvents.KEY_PRESSED_START,e.getKeyCode()));
		}
		for(int i = _keys.size() - 1; i >= 0; i--){
			this.dispatchEvent(new EventKeyboard(FrameEvents.KEY_PRESSED,_keys.get(i)));
		}
	}
	...
	public void keyReleased(KeyEvent e) {
		...
		this.dispatchEvent(new EventKeyboard(FrameEvents.KEY_RELEASED,e.getKeyCode()));
	}
	
	@Override
	public void keyTyped(KeyEvent e) {
		this.dispatchEvent(new EventKeyboard(FrameEvents.KEY_TYPED,e.getKeyCode()));
	}
