...
public class Timer extends EventDispatcher{
	...
	public void start(float timeInSeconds, int timesToRepeat){
		if(timeInSeconds > 0 && timesToRepeat >= 0){
			...
			GameEngine.getCoreListener().addEventListener(FrameEvents.ENTER_FRAME, getEventMethodData("tik"));
	...
	public void stop(){
		if(_timeGivenInSeconds != 0){
			GameEngine.getCoreListener().removeEventListener(FrameEvents.ENTER_FRAME, getEventMethodData("tik"));
			_timesRepeated = 0;
			_timePassedInSec = 0;
			_running = false;
			_paused = false;
